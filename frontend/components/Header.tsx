...
export default function Header() {
  ...
  return (
    <>
    <header className="fixed top-0 left-0 w-full h-20 bg-white shadow z-50 flex justify-between items-center p-4 bg-white">
      {/* Logo */}
      ...
      <div className="flex items-center gap-3 pr-4">
        {!isLoggedIn ? (
          <>
            <button onClick={() => setShowLogin(true)} className="px-4 py-2 text-sm border border-gray-300 rounded-md hover:bg-gray-100 cursor-pointer">Log In</button>
            <button onClick={() => setShowSignup(true)} className="px-4 py-2 text-sm text-white bg-blue-600 rounded-md hover:bg-blue-700 cursor-pointer">Sign Up</button>
          </>
        ) : (
          <>
